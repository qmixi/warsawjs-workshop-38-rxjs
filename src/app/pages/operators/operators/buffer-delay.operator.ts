...
export const bufferDelay =
  <T>(time: number, scheduler: SchedulerLike = asyncScheduler): MonoTypeOperatorFunction<T> => (source: Observable<T>): Observable<T> => {
    return new Observable<T>(observer => {
      ...
      zip(source, isReady$)
        .pipe(
          tap(() => {
            ready.next(false);
            scheduler.schedule(() => ready.next(true), time);
          }),
          map(([v]) => v),
        )
        .subscribe({
